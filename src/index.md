...
<style>
  * { box-sizing: border-box; }
  ...
  /* 1. Header - Fixed height, no shrinking */
  .header-block { 
    height: 80px; flex-shrink: 0; padding-top: 15px;
  }
  .big-header { 
    font-size: 34px; font-weight: bold; text-shadow: 0 0 15px #0f0; margin: 0; 
  }
  .status-line { font-size: 11px; color: #fff; margin-top: 5px; }
  
  /* 2. Nav - Fixed height, side-by-side wide layout */
  .nav-block { 
    height: 50px; flex-shrink: 0; display: flex; gap: 5px; padding: 5px 10px;
  }
  .nav-block a { 
    flex: 1; border: 1px solid #0f0; color: #0f0; text-decoration: none;
    display: flex; align-items: center; justify-content: center;
    font-size: 11px; font-weight: bold; background: rgba(0,255,0,0.05);
  }
  
  /* 3. THE TERMINAL - DYNAMIC EXPANSION */
  #terminal-output { 
    flex-grow: 1; /* Forces it to fill ALL available space */
    width: 96vw; margin: 5px auto; border: 2px solid #0f0;
    padding: 15px; overflow-y: auto; font-size: 14px;
    background: #000; text-align: left;
  }
  
  /* 4. Input - Fixed height at bottom */
  .input-block { 
    height: 70px; flex-shrink: 0; display: flex; padding: 10px; gap: 5px;
  }
  input { 
...
<div class="header-block">
  ...
  <div class="status-line">[ONLINE] | <span id="clock"></span> | [V.6.5 EXPANDED]</div>
</div>
...
<div id="terminal-output">
  <div class="ARES">[ARES-01]: System Wide-Screen Lock Active.</div>
  <div class="TITAN">[TITAN-06]: Terminal expanded to fill 100% of available depth.</div>
</div>
